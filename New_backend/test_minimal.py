...
def test_basic_scan(api_url, test_url):
    """
    Test the basic scanner functionality with minimal dependencies
    ...
    try:
        payload = {
            "url": test_url,
            "scanners": ["basic"],  # Only basic scanner for testing
        ...
        if response.status_code == 200:
            ...
            while not completed and polls < max_polls:
                ...
                if status_response.status_code == 200:
                    ...
                    print(f"  Poll {polls}: Status = {status}, Progress = {progress}%, Message = {message}")
                    
                    if status in ["completed", "failed"]:
                        completed = True
                        
                        # 3. Check results only if completed
                        if status == "completed":
